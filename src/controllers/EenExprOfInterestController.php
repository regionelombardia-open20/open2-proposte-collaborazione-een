...
class EenExprOfInterestController extends CrudController
{
    ...
    public function beforeAction($action)
    {
        if (\Yii::$app->user->isGuest) {
            $titleSection = AmosEen::t('amoseen', 'Manifestazioni di interesse EEN');
            $urlLinkAll   = '';
            ...
            $titleSection = AmosEen::t('amoseen', 'Manifestazioni di interesse EEN');
            $labelLinkAll = AmosEen::t('amoseen', 'Tutte');
            if(\Yii::$app->user->can('ADMIN') || \Yii::$app->user->can('STAFF_EEN')){
                $urlLinkAll = '/een/een-expr-of-interest/index-all';
            }
            $titleLinkAll = AmosEen::t('amoseen',
                    'Visualizza la lista delle manifestazioni di interesse alle proposte EEN');

            $subTitleSection = ''; //Html::tag('p', AmosEen::t('amoseen', '#beforeActionSubtitleSectionLogged'));
            $titleManage = AmosEen::t('amoseen', 'Gestisci le manifestazioni');
    ...
    private function setTitleAndBreadcrumbs($eenPageTitle)
    {
        ...
        $this->view->params['titleSection'] = $eenPageTitle;
    ...
    public function setNetworkDashboardBreadcrumb()
    {
        if (!empty($this->scope)) {
            if (isset($this->scope['community'])) {
                ...
                $dashboardCommunityTitle                 = AmosEen::t('AmosEen', "Dashboard").' '.$community->name;
...
//        $this->setCreateNewBtnLabel();
    ...
    private function setCreateNewBtnLabel()
    {
        ...
        Yii::$app->view->params['createNewBtnParams'] = [
            'createNewBtnLabel' => AmosEen::t('amoseen', 'Add new een'),
            'urlCreateNew' => [(array_key_exists("noWizardNewLayout", Yii::$app->params) ? '/een/een-expr-of-interest/create' : '/een/een-expr-of-interest/index-received')]
        ];
    }
    ...
    public function actionIndex($layout = NULL)
    {
        if (\Yii::$app->user->can('STAFF_EEN')) {
            $this->setTitleAndBreadcrumbs(AmosEen::t('amoseen', 'Ricevute'));
            return $this->redirect('/een/een-expr-of-interest/index-received');
        }
        $this->setTitleAndBreadcrumbs(AmosEen::t('amoseen', 'Di mio interesse'));
    ...
    public function actionIndexOwn($layout = NULL)
    {
        ...
        $this->view->params['titleSection'] = AmosEen::t('amoseen', 'Manifestazioni di interesse Inviate');
    ...
    public function actionIndexReceived($layout = NULL)
    {
        ...
        $this->setCurrentView($this->getAvailableView($this->getCurrentView()));
        $this->setTitleAndBreadcrumbs(AmosEen::t('amoseen', 'Ricevute'));
        ...
        if (!\Yii::$app->user->isGuest) {
//            $bc = \open20\amos\utility\models\BulletCounters::getAmosWidgetIconCounter(
//                \Yii::$app->getUser()->getId(), AmosEen::getModuleName(),
//                WidgetIconEenExprOfInterestReceived::class, false, null,
//                WidgetIconEenExprOfInterestReceived::class, true
//            );
//            \open20\amos\utility\models\BulletCounters::getAmosWidgetIconCounter(
//                \Yii::$app->getUser()->getId(), AmosEen::getModuleName(),
//                WidgetIconEenExprOfInterestReceived::class, true, null,
//                WidgetIconEenExprOfInterestReceived::class, true
//            );
//            $this->view->params['titleSection'] = AmosEen::t('amoseen', 'Ricevute');
//            $this->view->params['bulletCount'] = $bc;
    ...
    public function actionIndexAll($layout = NULL)
    {
        ...
        if (!\Yii::$app->user->isGuest) {
            $bc = \open20\amos\utility\models\BulletCounters::getAmosWidgetIconCounter(
                \Yii::$app->getUser()->getId(), AmosEen::getModuleName(),
                WidgetIconEenExprOfInterestAll::class, false, null,
                WidgetIconEenExprOfInterestAll::class, true
            );
            \open20\amos\utility\models\BulletCounters::getAmosWidgetIconCounter(
                \Yii::$app->getUser()->getId(), AmosEen::getModuleName(),
                WidgetIconEenExprOfInterestAll::class, true, null,
                WidgetIconEenExprOfInterestAll::class, true
            );

            $this->view->params['titleSection'] = AmosEen::t('amoseen', 'Tutte le manifestazioni');
            $this->view->params['bulletCount'] = $bc;

             $this->view->params['labelLinkAll'] = AmosEen::t('amoseen', 'Ricevute');
            $this->view->params['urlLinkAll']   = '/een/een-expr-of-interest/index-received';
            $this->view->params['titleLinkAll'] = AmosEen::t('amoseen',
    ...
    public function actionCreate($idPartnershipProposal, $request_more_info = 0)
    {
        ...
        if (!\open20\amos\een\models\EenExprOfInterest::canCreateExpressionOfInterest($idPartnershipProposal)) {
            throw new ForbiddenHttpException(AmosEen::t('amoseen',
                'Non è permessso effettuare questa manifestazione di interesse.'));
        }
        /** @var  $model EenExprOfInterest */
            ...
            if ($model->save()) {
                ...
                if ($model->is_request_more_info == 0) {
                    Yii::$app->getSession()->addFlash('success',
                        AmosEen::t('amoseen', 'Expression of interest created correctly'));
                } else {
                    Yii::$app->getSession()->addFlash('success',
                        AmosEen::t('amoseen', 'Information request created correctly'));
                }
...
//        }
    ...
    public function actionStaffEen()
    {
        ...
        $this->view->params['disable-search']  = true;
        ...
        if (!\Yii::$app->user->isGuest) {
            $this->view->params['titleSection'] = AmosEen::t('amoseen', 'Staff EEN');
...
//                pr($model->getErrors());
    ...
    public function actionPdf($id, $save = false)
    {
        ...
        $content = $this->renderPartial('@vendor/open20/amos-proposte-collaborazione-een/src/views/een-expr-of-interest/_view_pdf',
            ['model' => $eenExpr, 'profile' => $profile]);
        $footer  = $this->renderPartial('@vendor/open20/amos-proposte-collaborazione-een/src/views/een-expr-of-interest/_pdf_footer',
            ['model' => $eenExpr]);
...
//        $pdf->getApi()->SetAutoPageBreak(TRUE, 25);
        ...
        if ($save) {
            return $pdf->output($content, \Yii::$app->basePath."/runtime/mpdf/tmp/Expression_of_interest_een.pdf",
                    Pdf::DEST_FILE);
        } else {
...
//        $pdf->getApi()->SetAutoPageBreak(TRUE, 25);
    ...
    public static function getManageLinks()
    {
        ...
        if (\Yii::$app->user->can(\open20\amos\een\widgets\icons\WidgetIconEenExprOfInterest::class)) {
            $links[] = [
                'title' => AmosEen::t('amoseen', '#expr_of_interest_sended'),
                'label' => AmosEen::t('amoseen', '#expr_of_interest_sended'),
                'url' => '/een/een-expr-of-interest/index-own'
            ];
        }
        if (\Yii::$app->user->can(\open20\amos\een\widgets\icons\WidgetIconEenExprOfInterestReceived::class)) {
            $links[] = [
                'title' => AmosEen::t('amoseen', 'Ricevute'),
                'label' => AmosEen::t('amoseen', 'Ricevute'),
                'url' => '/een/een-expr-of-interest/index-received'
            ];
        }

        if (\Yii::$app->user->can(\open20\amos\een\widgets\icons\WidgetIconEenExprOfInterestAll::class)) {
            $links[] = [
                'title' => AmosEen::t('amoseen', 'Tutte'),
                'label' => AmosEen::t('amoseen', 'Tutte'),
                'url' => '/een/een-expr-of-interest/index-all'
            ];
        }

        if (\Yii::$app->user->can(\open20\amos\een\widgets\icons\WidgetIconEenStaff::class)) {
            $links[] = [
                'title' => AmosEen::t('amoseen', 'Visualizza la lista dello staff EEN'),
                'label' => AmosEen::t('amoseen', 'Staff EEN'),
                'url' => '/een/een-expr-of-interest/staff-een'
            ];
        }
