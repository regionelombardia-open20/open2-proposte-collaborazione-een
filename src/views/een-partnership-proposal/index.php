...
if (($profile && !$profile->validato_almeno_una_volta)) {
    ...
    echo "<p>" . \open20\amos\een\AmosEen::t('amoseen', "Solo i partecipanti con un profilo completo e validato possono pubblicare proposte di collaborazione segui <a href='{link}'>questo link</a> per completare il profilo.",
            [
                'link' => '/' . AmosAdmin::getModuleName() . '/user-profile/update?id=' . $profile->id
            ]) . "</p>";
...
<div class="proposte-collaborazione-een-index">
    ...
    <?= DataProviderView::widget(['dataProvider' => $dataProvider,
    //'filterModel' => $model,
    'currentView' => $currentView,
    'gridView' => ['columns' => ['content_title',
    'reference_external',
    'reference_type',
    /*
    ...
    */
    ...
    'buttons' => ['expr_of_interest' => function($url, $model)use ($isArchived){
    if (!$model->isExprOfInterestSended() && !$isArchived) {
    return \open20\amos\core\helpers\Html::a(\open20\amos\core\icons\AmosIcons::show('thumb-up'), "#interestPopup-{
    $model->id
    }", ['class' => ['btn btn-tools-secondary'],
    'title' => \open20\amos\een\AmosEen::t('amoseen', '#expr_of_interest_verb'),
    'data-target' => "#interestPopup-{
    $model->id
    }",
    'data-toggle' => "modal",]) . $this->render('_modal_expr_of_interest', ['model' => $model]);
    }
    else return '';
    }]],],],
    'listView' => ['itemView' => '_item',
    'masonry' => FALSE,
